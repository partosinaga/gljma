	...
	public function get_ap(){
		$data = $this->db->query("
			SELECT
				...
				ap_header.Fmonth,
				ap_header.gl_date,
				ap_header.receive_from,
				ap_header.posted_no,
				ap_header.no_cek,
				gl_header.status as gl_status,
				gl_header.Fclose
			FROM
				ap_header
			LEFT JOIN gl_header ON gl_header.reff_no = ap_header.no_voucher
			AND ap_header.description = gl_header.description
			AND ap_header.total = gl_header.total
			WHERE 
				ap_header.status != 'cancel'
			ORDER BY
				ap_header.id DESC
			;");
		return $data->result();

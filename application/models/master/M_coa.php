...
class M_coa extends CI_Model{
	...
	public function get_data_edit($coa_id){
		$data= $this->db->query("select * from coa where coa_id = '".$coa_id."'");
		return $data->result();
	}

	public function save_edit_coa($coa_id,$name_coa,$date,$subgroup,$debit,$credit,$header,$active){
		$data = $this->db->query(" update coa set name_coa = '".$name_coa."' , date = '".$date."' , subgroup = '".$subgroup."' , debit = '".$debit."' , credit = '".$credit."' , header = '".$header."' ,active = '".$active."' WHERE coa_id = '".$coa_id."' ");
	}

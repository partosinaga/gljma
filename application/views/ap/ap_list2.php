                ...
                <table id="example" class="table table-striped table-bordered" cellspacing="0">
                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td><?php $d=New DateTime($ap->date); echo $d->format('d-m-Y') ?></td>
                          <td><?php echo $ap->description ?></td>
                          <td><?php echo $ap->receive_from ?></td>
                          <td align="center">
                            <?php if($ap->status=='posted'){ ?>
                              <p title="This voucher have been posted">
                                <?php  echo $ap->no_cek?>  
                              </p>
                            <?php }else{ ?>
                              <div class="input-group input-group-sm">
                                <input type="text" class="form-control" id="<?php echo $ap->no_voucher ?>" name="no_cek" readonly="" value="<?php echo $ap->no_cek ?>">
                                <span class="input-group-btn ">
                                    <button class="btn grey enabled" nov="<?php echo $ap->no_voucher ?>" type="button"><i id="<?php echo $ap->no_voucher ?>" class=" fa fa-pencil"></i></button>
                                </span>
                              </div>
                            <?php } ?>
                          ...
                          <td align="center" >
                            ...
                            <?php } else { ?> 
                              ...
                              <a style="color: black">
                              <button class="select_date" nov ="<?php echo $ap->no_voucher ?>" style="background-color: transparent;border: 0" >  
                                <?php  
                                  $date = New DateTime($ap->gl_date);
                                  echo $date->format("d-m-Y");
                                ?>  
                           ...
                           <div class="btn-group">
                            <button class="btn red blue-hoki-stripe btn-xs dropdown-toggle" data-toggle="dropdown"><i class="angle-down"></i> Action
                              ...
                              <ul class="dropdown-menu">
                                ...
                                <li>
                                  ...
                                  <?php
                                    ...
                                    if ($ap->total >= 50000000 ) {
                                      $result .= '<a href ="'.site_url('/ap/ap/print_ap_up?id=').$ap->no_voucher.'" target="_blank"  "><i class="fa fa-print"></i> Print
                                        </a>';
                                    }else{
                                       $result .= '<a href ="'.site_url('/ap/ap/print_ap?id=').$ap->no_voucher.'" target="_blank"  "><i class="fa fa-print"></i> Print
                                        </a>';
                                    }
                                    echo $result;
                                  ?>
                                ...
                                <?php
                                  $result = '<li>';  
                                  if ($ap->status == "posted" OR $ap->Fclose == "close" OR $ap->Fmonth == 'close') {
                                    echo '
                                      <li>
                                      </li>
                                    ';
                                    ...
                                    $result .= 
                                    '<a href='.site_url('ap/ap/edit_ap?id=').$ap->no_voucher.' >
                                ...
                                ?>
                                <li>
                                  <a href="<?php echo site_url('ap/ap/cancel_ap?id='.$id.'&status='.$ap->status) ?>"><i class="fa fa-remove"></i> Cancel </a>
                                </li>
                                <!-- <li>
...
<div id="glmodal" class="modal fade  modale"  data-backdrop="static" data-keyboard="false">
  ...
  <div class="modal-footer" >
    <button type="button" data-dismiss="modal" class="btn red blue-hoki-stripe btn-sm"><i class="fa fa-remove"></i> Cancel</button>
  ...
  var nv = $(this).attr("nov");
    ...
    $('#glmodal').modal('show');
      $('.save').on("click", function(e){
        ...
        $.ajax({
          type:'POST',
          url: '<?php echo site_url();?>/ap/ap/edit_glDate',
          data: {
            nv: nv,
            hari: hari,
            bulan: bulan,
...
$('.enabled').on("click", function(){
  ...
  if(input.hasAttribute('readonly') == true){// if readonly
      ...
      $.ajax({
        method: 'POST',
        url: '<?php echo site_url() ?>/ap/ap/edit_cek',
        data: {
          nov: nov,
          value: value
        },
